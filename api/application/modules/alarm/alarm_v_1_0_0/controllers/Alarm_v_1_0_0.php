...
class Alarm_v_1_0_0 extends MY_Controller {
    ...
    $member_idx = $this->_input_check("member_idx",array("empty_msg"=>"회원 키가 누락되었습니다."));
		...
		$result = $this->model_alarm->new_alarm_count($data); // 알림 삭제
		...
		if($result < 0){
			...
			$response->new_alarm_count = $result;
		}
  ...
  public function alarm_list(){
    ...
    $member_idx = $this->_input_check("member_idx",array("empty_msg"=>"회원 코드가 누락되었습니다."));
		...
		for ($i=0; $i < count($result_list); $i++) {
			if ($i==0) {
				$alarm_idxs = $result_list[$i]->alarm_idx;
			} else {
				$alarm_idxs = $alarm_idxs.",".$result_list[$i]->alarm_idx;
			}
		}
		$data['alarm_idxs'] = $alarm_idxs;

		$alarm_read = $this->model_alarm->alarm_read($data); // 조회한 알림 읽음
  ...
  public function alarm_toggle_view(){
	...
	public function alarm_toggle(){
		header('Content-Type: application/json');
    $member_idx = $this->_input_check("member_idx",array("empty_msg"=>"회원 코드가 누락되었습니다."));
    $all_alarm_yn = $this->_input_check("all_alarm_yn",array("empty_msg"=>"알람정보가 누락되었습니다."));
