...
class Member_pw_change_v_1_0_0 extends MY_Controller {
  ...
  public function member_pw_mod_up(){
    ...
    $new_member_pw = $this->_input_check("new_member_pw",array("empty_msg"=>"비밀번호를 입력해주세요.","regular_msg" => "비밀번호는 영어,숫자 조합으로 8자~15자내로 입력해주세요.","type" => "custom","custom" => "/^[a-zA-Z](?=.{0,15}[0-9])[0-9a-zA-Z]{8,15}$/"));
    ...
    if($pwd_check_result == 0){
      $response->code = "-1";
      $response->code_msg = "기존 비밀번호가 일치하지 않습니다.";
      ...
      exit;
		...
		if($new_member_pw != $new_member_pw_check){
			$response->code = "-1";
			$response->code_msg = "새 비밀번호와 새 비밀번호 확인이 일치 하지 않습니다. 다시 확인해 주세요.";
